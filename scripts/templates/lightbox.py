...
(function() {
  ...
  function metaPanel(a) {
    ...
    var ref = (local && (String(local).indexOf('http') === 0 ? local : './' + local)) || url || src || '';
    var base = ref ? decodeURIComponent(String(ref).split('?')[0].split('#')[0].split('/').pop() || '') : '';
    var dot = base.lastIndexOf('.');
    var ext = dot > 0 ? base.slice(dot) : '';
    var stem = ext ? base.slice(0, -ext.length) : base;
    var RELEASE_BY_DATE = { '5/8/26': 'Release 01', '5/22/26': 'Release 02' };
    var releaseLabel = releaseDate
      ? (RELEASE_BY_DATE[releaseDate] ? releaseDate + ' — ' + RELEASE_BY_DATE[releaseDate] : releaseDate)
      : (release ? 'Release ' + release : '');
    var docType = ext ? ext.toLowerCase()
      : (type === 'PDF' ? '.pdf'
       : (type === 'VID' || type === 'VID-FILE' ? '.mp4'
       : (type === 'AUD' ? '.mp3'
       : ((type === 'IMG' || type === 'SLIDE') ? '.jpg' : type))));

    var rows = [];
    function add(k,v){ if (v) rows.push('<div><dt>'+esc(k)+'</dt><dd>'+esc(v)+'</dd></div>'); }
    if (stem) add('Asset File Name', stem);
    if (releaseLabel) add('Release', releaseLabel);
    add('Agency', agency);
    add('Incident Date', date || 'N/A');
    add('Incident Location', location || 'N/A');
    add('Document Type', docType);
    ...
    var links = [];
    var openUrl = url || (local && (String(local).indexOf('http') === 0 ? local : './' + local));
    if (openUrl) links.push('<a href="' + esc(openUrl) + '" target="_blank" rel="noopener" download>Download</a>');
    if (src && src !== openUrl) links.push('<a href="' + esc(src) + '" target="_blank" rel="noopener">Source &#8599;</a>');
