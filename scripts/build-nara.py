#!/usr/bin/env python3
"""Build nara-mirror/index.html — curated NARA UAP records gateway.
...
ROOT = os.path.join(REPO, 'nara-mirror')


def git_tracked(rel_dir):
    try:
        out = subprocess.run(
            ['git', '-C', REPO, 'ls-files', f'nara-mirror/{rel_dir}/'],
            capture_output=True, text=True, check=True,
        ).stdout
        prefix = f'nara-mirror/{rel_dir}/'
...
<link rel="canonical" href="https://realufo.org/nara-mirror/">
...
<meta property="og:url" content="https://realufo.org/nara-mirror/">
